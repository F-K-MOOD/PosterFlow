...
export default class UserService extends Service {
  ...
  async loginByCellphone(cellphone: string) {
    ...
    const token = app.jwt.sign({ username: newUser.username }, app.config.jwt.secret, { expiresIn: app.config.jwtExpires })
    return token
  }
  // get access token
  ...
  async loginByGitee(code: string) {
    ...
    if (existUser) {
      const token = app.jwt.sign({ username: existUser.username }, app.config.jwt.secret, { expiresIn: app.config.jwtExpires })
      return token
    }
    // 假如不存在，新建用户
    const userCreatedData: Partial<UserProps> = {
  ...
  async updateUser(username: string, updateData: Partial<UserProps>) {
    const { ctx } = this
